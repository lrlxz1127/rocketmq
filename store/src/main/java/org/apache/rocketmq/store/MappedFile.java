...
public class MappedFile extends ReferenceResource {
	...
	protected final AtomicInteger wrotePosition = new AtomicInteger(0);// 写位点
	// ADD BY ChenYang
	protected final AtomicInteger committedPosition = new AtomicInteger(0);// 提交偏移量
	private final AtomicInteger flushedPosition = new AtomicInteger(0);// 缓冲区偏移量
	protected int fileSize;// 文件大小
	protected FileChannel fileChannel;// 文件信道（读写模式）
	/**
	 * Message will put to here first, and then reput to FileChannel if
	 * writeBuffer is not null.
	 */
	protected ByteBuffer writeBuffer = null;// 内存缓冲区
	protected TransientStorePool transientStorePool = null;//
	private String fileName;// 文件名
	private long fileFromOffset;// 文件名，既是文件偏移开始位点
	private File file;// 文件对象
	private MappedByteBuffer mappedByteBuffer;// 文件字节缓存区
	private volatile long storeTimestamp = 0;
	private boolean firstCreateInQueue = false; // 在队列中是否为第一个创建的
