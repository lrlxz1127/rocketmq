...
public abstract class NettyRemotingAbstract {
	...
	/**
	 * This container holds all processors per request code, aka, for each incoming
	 * request, we may look up the responding processor in this map to handle the
	 * request.
	 */
			...
			Runnable run = new Runnable() {
				...
				public void run() {
					try {
						...
						final RemotingCommand response = pair.getObject1().processRequest(ctx, cmd);// respose返回null就不通知消费端
	...
	/**
	 * Custom RPC hook. Just be compatible with the previous version, use
	 * getRPCHooks instead.
	 */
	@Deprecated
	 ...
	 *
	 * @return Dedicated thread pool instance if specified; or null if the callback
	 *         is supposed to be executed in the netty event-loop thread.
	 */
	...
	/**
	 * mark the request of the specified channel as fail and to invoke fail callback
	 * immediately
	 * 
