...
public class IndexFile {
	...
	private static int hashSlotSize = 4;// 单个hash槽的大小
	private static int indexSize = 20;//
	private static int invalidIndex = 0;
	private final int hashSlotNum;
	private final int indexNum;// 单个文件index的最大个数
	...
	/**
	 * 
	 * @param key            topic@messageId
	 * @param phyOffset      commitLog物理位点
	...
	public boolean putKey(final String key, final long phyOffset, final long storeTimestamp) {
		if (this.indexHeader.getIndexCount() < this.indexNum) {
			...
			int slotPos = keyHash % this.hashSlotNum;// 槽位点
	...
	public void selectPhyOffset(final List<Long> phyOffsets, final String key, final int maxNum, final long begin,
		...
		if (this.mappedFile.hold()) {
			...
			try {
				...
				int slotValue = this.mappedByteBuffer.getInt(absSlotPos);// IndexCount
				// if (fileLock != null) {
				// fileLock.release();
				// fileLock = null;
				// }
