...
public class RouteInfoManager {
	...
	private final static long BROKER_CHANNEL_EXPIRED_TIME = 1000 * 60 * 2;// broker信道过期时间
	...
	public RegisterBrokerResult registerBroker(final String clusterName, final String brokerAddr,
		...
		try {
			try {
				...
				// Switch slave to master: first remove <1, IP:PORT> in namesrv, then add <0,
				// IP:PORT>
	...
	public void onChannelDestroy(String remoteAddr, Channel channel) {
		String brokerAddrFound = null;// broker地址
		if (channel != null) {
			try {
				try {
					this.lock.readLock().lockInterruptibly();
		...
		if (null == brokerAddrFound) {// 如果在存活的broker列表中，没有发现broker。则已远程地址作为broker地址
		...
		if (brokerAddrFound != null && brokerAddrFound.length() > 0) {
			...
			try {
				try {
					this.lock.writeLock().lockInterruptibly();
					this.brokerLiveTable.remove(brokerAddrFound);// 移除broker存活列表
					this.filterServerTable.remove(brokerAddrFound);// 移除过滤器服务列表
					String brokerNameFound = null;// broker名称
					boolean removeBrokerName = false;// 标志broker名称是否被移除
