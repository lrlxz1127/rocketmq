 ...
 * underlying pull service. Specifically, on arrival of messages pulled from
 * brokers, it roughly invokes the registered callback handler to feed the
 * messages.
 * </p>
 *
...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
	 ...
	 * clustering is set, consumer clients with the same {@link #consumerGroup}
	 * would only consume shards of the messages subscribed, which achieves load
	 * balances; Conversely, if the broadcasting is set, each consumer client will
	 * consume all subscribed messages separately.
	 * </p>
	 *
	 * This field defaults to clustering.
	 */
	// 消息模式：集群模式和广播模式。
	// 集群模式：同一个consumerGroup、同一个topic下，一条消息只会被一个consumer消费
	// 广播模式：同一个consumerGroup、同一个topic下，一条消息会被所有的consumer消费
	 ...
	 * stopped previously. If it were a newly booting up consumer client, according
	 * aging of the consumer group, there are two cases:
	 * <ol>
	 * <li>if the consumer group is created so recently that the earliest message
	 * being subscribed has yet expired, which means the consumer group represents a
	 * lately launched business, consuming will start from the very beginning;</li>
	 * <li>if the earliest message being subscribed has expired, consuming will
	 * start from the latest messages, meaning messages born prior to the booting
	 * timestamp would be ignored.</li>
	 * </ol>
	 * </li>
	 * <li><code>CONSUME_FROM_FIRST_OFFSET</code>: Consumer client will start from
	 * earliest messages available.</li>
	 * <li><code>CONSUME_FROM_TIMESTAMP</code>: Consumer client will start from
	 * specified timestamp, which means messages born prior to
	 * {@link #consumeTimestamp} will be ignored</li>
	 * </ul>
	 */
	// 注意：如果从消息进度服务 OffsetStore 读取到 MessageQueue 中的偏移量不小于 0，则 使用读取到的偏移量，只有在读到的偏移量小于
	// 0 时，策略才会生效
	...
	/**
	 * Flow control threshold on queue level, each message queue will cache at most
	 * 1000 messages by default, Consider the {@code pullBatchSize}, the
	 * instantaneous value may exceed the limit
	 */
	// 流控-消息尺寸大小
	private int pullThresholdForQueue = 1000;

	/**
	 * Limit the cached message size on queue level, each message queue will cache
	 * at most 100 MiB messages by default, Consider the {@code pullBatchSize}, the
	 * instantaneous value may exceed the limit
	 *
	 * <p>
	 ...
	 * queues are assigned to this consumer, then pullThresholdForQueue will be set
	 * to 100
	 */
	 ...
	 * calculated based on {@code pullThresholdSizeForTopic} if it is't unlimited
	 * <p>
	...
	/**
	 * Constructor specifying consumer group, enabled msg trace flag and customized
	 * trace topic name.
	 *
	 * @param consumerGroup        Consumer group.
	 * @param enableMsgTrace       Switch flag instance for message trace.
	 * @param customizedTraceTopic The name value of message trace topic.If you
	 *                             don't config,you can use the default trace topic
	 *                             name.
	 */
	public DefaultMQPushConsumer(final String consumerGroup, boolean enableMsgTrace,
	 ...
	 *
	 * This method will be removed or it's visibility will be changed in a certain
	 * version after April 5, 2020, so please do not use this method.
	 *
	...
	/**
	 * This method gets internal infrastructure readily to serve. Instances must
	 * call this method after configuration.
	 *
	 * @throws MQClientException if there is any client error.
	 */
	@Override
	 ...
	 * @param subExpression subscription expression.it only support or operation
	 *                      such as "tag1 || tag2 || tag3" <br>
