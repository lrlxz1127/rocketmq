...
public class CommitLog {
	...
	// If TransientStorePool enabled, we must flush message to FileChannel at fixed
	// periods
	...
	public SelectMappedBufferResult getData(final long offset, final boolean returnFirstOnNotFound) {
		...
		if (mappedFile != null) {
			int pos = (int) (offset % mappedFileSize);// 获取文件的位点
	...
	public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
		...
		if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE || tranType == MessageSysFlag.TRANSACTION_COMMIT_TYPE) {
			// Delay Delivery
			if (msg.getDelayTimeLevel() > 0) {// 延迟队列
	...
	class CommitRealTimeService extends FlushCommitLogService {
		...
		private long lastCommitTimestamp = 0;// 上一次提交时间
	...
	class DefaultAppendMessageCallback implements AppendMessageCallback {
		...
		private static final int END_FILE_MIN_BLANK_LENGTH = 4 + 4;// 文件最后预留的最小尺寸
		private final ByteBuffer msgIdMemory;
		// Store the message content
		private final ByteBuffer msgStoreItemMemory;
		// The maximum length of the message
		private final int maxMessageSize;// 消息最大大小，默认4K
		// Build Message Key
		...
		public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
			...
			// PHY OFFSET 物理位点=文件开始位点+缓存区位点
			long wroteOffset = fileFromOffset + byteBuffer.position();

			this.resetByteBuffer(hostHolder, 8);// 重置host主机缓存

			// offsetMsgId 主机ip+端口信息+物理位点
			String msgId = MessageDecoder.createMessageId(this.msgIdMemory, msgInner.getStoreHostBytes(hostHolder),
					wroteOffset);
